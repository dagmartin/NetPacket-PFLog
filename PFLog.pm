...
sub decode {
	...
	my ($len, $af, $action, $reason, $ifname, $ruleset, $rulenr,
		...
		$rewritten, $pad, @src_addr, @dst_addr, $src_port, 
		$dst_port, $data);
	...
	if (defined $pkt) {
		($len, $af, $action, $reason, $ifname, $ruleset, $rulenr, 
			$subrulenr, $uid, $pid, $rule_uid, $rule_pid, $dir, 
			$rewritten, $pad, @src_addr[0..3], @dst_addr[0..3], 
			$src_port, $dst_port, $data) =
			unpack("CCCCa16a16NNIiIiCCa2NNNNNNNNnna*", $pkt);
...
sub encode {
	...
	my $packet = pack("CCCCa16a16NNIiIiCCa2NNNNNNNNnna*",
		...
		$self->{pad}, @src_ip[0..3], @dst_ip[0..3], 
...
Indicates if the packet was rewritten (NAT)

=item src_ip

The original source IP before the packet was rewritten

=item dst_ip

The original destination IP before the packet was rewritten

=item src_port

The original source port before the packet was rewritten

=item dst_port

The original destination port before the packet was rewritten
